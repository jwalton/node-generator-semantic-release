...
module.exports = class extends (
    ...
    async prompting() {
        ...
        Object.assign(
            ...
            await this.prompt({
                name: 'ciService',
                type: 'list',
                message: 'Which CI service are you using?',
                choices: ['Travis', 'CircleCI', 'Other'],
            })
        );
    }
    ...
    async writing() {
        ...
        if (this.props.ciService === 'Travis') {
            await this.generateTravisConfig();
        }

        if (this.props.releaseConfig) {
            this.fs.write(
                this.destinationPath('.releaserc.yml'),
                `extends: '${this.props.releaseConfig}'`
            );
        }
    }
    ...
    install() {
      ...
      if (this.props.ciService === 'Travis') {
        ...
        this.log('Add the following environment variables to your CI project config:');
        this.log('');
        this.log(`    GH_TOKEN="${this.props.githubToken}"`);
        this.log(`    NPM_TOKEN="${this.props.npmToken}"`);
        this.log('');
        this.log('Then add the following commands somewhere in your .circleci/config.yml:');
        this.log('');
        this.log('    npm run semantic-release');
        this.log('');
      }
    ...
    async generateTravisConfig() {
        ...
        try {
            ...
            travisConfig.jobs.include = travisConfig.jobs.include.filter(
            ...
            this.log('Could not add configuration to .travis.yml.');
        }
    }
};
